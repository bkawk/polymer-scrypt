 ...
 */
class PolymerScrypt extends PolymerElement {
  static get template() {
    return html`
      <style>
        :host {
  ...
  _start(){
    if(this.password && this.password.length >= 8){
      this._scrypt(this.password)
    } else {
      this.error = 'The password must be atleast 8 charectors'
    }
  }

  _scrypt(password){
    return new Promise((resolve, reject) => {
      this.scrypt(password, 'SALT_ME', {
        N: 16384,
        r: 8,
        p: 1,
        dkLen: 16,
  ...
  constructor() {
    ...
    this.scrypt = webpack.scrypt
  }

} window.customElements.define('polymer-scrypt', PolymerScrypt);
